...
softplus_4_mix = tf.layers.dense(tf.concat(softplus_3,multinomial_2),128, use_bias=True,activation=tf.nn.softplus,name='nn_softplus_layer_4')
...
relu_4 = tf.layers.dense(trelu_3,128,use_bias=True,activation=tf.nn.relu,name='nn_relu_layer_4')
relu_4_mix = tf.layers.dense(tf.concat(relu_3,multinomial_3),128,use_bias=True,activation=tf.nn.relu,name='nn_relu_layer_4')


mixing_1 = tf.layers.dense(tf.concat(softplus_1,relu_1),1028,use_bia=True,activation=tf.sigmoid,name='nn_mixing_layer_1)
mixing_2 = tf.layers.dense(tf.concat(mixing_1,softplus_1,relu_2),528,use_bia=True,activation=tf.sigmoid,name='nn_mixing_layer_1)
mixing_3 = tf.layers.dense(tf.concat(mixing_2,softplus_3,relu_3),128,use_bia=True,activation=tf.sigmoid,name='nn_mixing_layer_1)
mixing_3_mix = tf.layers.dense(tf.concat(mixing_2,softplus_3,relu_3,multinomial_2,multinomial_3),128,use_bia=True,activation=tf.sigmoid,name='nn_mixing_layer_1)

model_output = tf.layers.dense(tf.concat(multinomial_1,softplus_4,relu_4,mixing_3,softplus_4_mix,relu_4_mix,mixing_3_mix),128,use_bias=False,actiavtion=None)
        ...
        try:
            ...
            if((__i)%FLAGS.iter_check == 0):
                ...
                if(current_validation > best_validation):
                    ...
                    saver.save(sess=sess, save_path=FLAGS.summary_dir + '/bestNetwork/model.ckpt')
